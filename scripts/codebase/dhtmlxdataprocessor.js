...
function dataProcessor(serverProcessorURL){
	...
	this.mandatoryMessages = new Array(0); //array of messages to invalidated fields
	...
	dataProcessor.prototype.checkBeforeUpdate = function(rowId,updateFl){
		...
		for(var i=0;i<this.mandatoryFields.length;i++){
			if(this.mandatoryFields[i]){
				...
				var message = this.mandatoryMessages[i];
				if (message == "undefined") {message = " Incorrect ";}
				if((typeof(this.mandatoryFields[i])=="function" && this.mandatoryFields[i](val,colName,message)) || (typeof(this.mandatoryFields[i])!="function" && val.toString()._dhx_trim()!="")){
...
*/
	...
	*	@type: public;
	*/
	dataProcessor.prototype.setVerificator = function(ind,verifFunction,message){
		if(verifFunction){
			this.mandatoryFields[ind] = verifFunction;
			this.mandatoryMessages[ind] = message;
		}else
