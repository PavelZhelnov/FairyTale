...
class UploadSession {
  ...
  function __construct($post, $files, $server) {
    $this->_post = empty($post) ? $_POST : $post;
    $this->_files = empty($files) ? $_FILES : $files;
    $this->_server = empty($server) ? $_SERVER : $server;
  }
  ...
  public function processRequest() {
    ...
    if ($package->getCompleted()) {
      ...
      if (!empty($this->_uploadCache) && (empty($this->_allFilesUploadedCallback) || $packageCount - 1 == $packageIndex)) {
        $this->getUploadCache()->cleanUploadSessionCache();
      }
    }
  }
  ...
  public function getUploadCache() {
    if ($this->_uploadCache == NULL) {
      $this->_uploadCache = new UploadCache($this->getUploadSessionId());
    }
