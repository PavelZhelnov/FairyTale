...
class Package {
  ...
  function __construct($uploadSession, $index, $saveIntoCache = false) {
    ...
    if ($index < $post[PostFields::packageIndex]) {
      ...
      $this->_cached = $cache->isPackageCached($this->_packageIndex);
      ...
      if (!$this->_completed || $this->_cached || $saveIntoCache) {
        $cache->saveRequestData($this->_packageIndex, $post, $files);
        $this->_cached = true;
      } else {
  ...
  private function loadSavedFields() {
    $cache = $this->_uploadSession->getUploadCache();
    $this->_packageFields = $cache->loadSavedFields($this->_packageIndex);
  }

  private function loadSavedFiles() {
    $cache = $this->_uploadSession->getUploadCache();
    $this->_packageFiles = $cache->loadSavedFiles($this->_packageIndex);
  }
