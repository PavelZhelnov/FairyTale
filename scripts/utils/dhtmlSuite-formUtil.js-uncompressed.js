...
DHTMLSuite.formValidator=function(propArray){
	...
	var domainString='(biz|com|org|net|mil|edu|info|mobi|travel|a[cdfgilmnoqrstuwxz]|b[abdefghijmnorstwyz]|c[acdfghiklmnoruvxyz]|d[ejkmoz]|e[cegrstu]|f[ijkmor]|g[adefghilmnpqrstuwy]|h[kmnrtu]|i[delmnoqrst]|j[emop]|k[eghimnrwyz]|l[abcikrstuvy]|m[acdghklmnopqrstuvwxyz]|n[acefgilopruz]|om|p[aefghklmnrstwy]|qa|r[eouw]|s[abcdeghiklmnrtvyz]|t[cdfghjklmnortvwz]|u[agksyz]|v[aceginu]|w[fs]|y[etu]|z[amw])';
	var amPmString='(AM|PM)';

	this.masks.email=new RegExp("^([A-Z0-9._%-]+@[A-Z0-9.-]+\\."+domainString+")((([,;] ?){1})([A-Z0-9._%-]+@[A-Z0-9.-]+\\."+domainString+"))*$","gi");
	this.masks.datetime=new RegExp("^[0-9]{2}[\\/][0-9]{2}[\\/][0-9]{4}([ ]){1}([0-9]{2}[\\:][0-9]{2})+$","gi");
	this.masks.date=new RegExp("^[0-9]{2}[\\/][0-9]{2}[\\/][0-9]{4}$","gi");
	this.masks.time=new RegExp("^[0-9]{2}[\\:][0-9]{2}([ ]){1}"+amPmString+"$","gi");
	this.masks.numeric=/^[0-9]+$/gi;
	this.masks.numeric_double=/^[0-9\,\.]+$/gi;
	this.masks.phone=/^[()0-9\.-]+$/gi;
	this.masks.price=/^[0-9]+|(TBA)$/gi;	
// Numeric
...
// {{{ __validatePaste()
	...
	*@private
	...
	if(pat=='numeric_double')src.value=src.value.replace(/[^0-9\.\,]/g,'');
	if(pat=='phone')src.value=src.value.replace(/[^0-9\.-]/g,'');
	if(pat=='price')src.value=src.value.replace(/[^0-9\.-]/g,'');
	if(pat=='datetime')src.value=src.value.replace(/[^0-9]/g,'');	
	if(pat=='date')src.value=src.value.replace(/[^0-9\.-]/g,'');
	if(pat=='time')src.value=src.value.replace(/[^A-Z0-9\.-]/g,'');
	if(pat=='letter'||pat=='numeric'||pat=='numeric_double'||pat=='price'||pat=='phone'||pat=='datetime'||pat=='date'||pat=='time')this.__validateInput(elRef);
	}
...
// Scandinavian characters
	...
	if(pat=='numeric_double'&&!key.match(/[0-9\.\,]/g))return false;
	if(pat=='price'&&!key.match(/[0-9\.\,]/g))return false;
	if(pat=='phone'&&!key.match(/[0-9\.-]/g))return false;
	if(pat=='datetime'&&!key.match(/[0-9 \/\:]/g))return false;
	if(pat=='date'&&!key.match(/[0-9 \/\:]/g))return false;
	return true;	
// Not yet implemented
	}
